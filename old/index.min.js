    ...
    link.rel = 'stylesheet';
    link.href = 'https://cdn.jsdelivr.net/gh/ahm23/CULearn@1.2.3a/index.min.css';
...
async function retrieveRaw(url) {
    let rawDat;
...
function renderCourses() {
    for (group of $('.courses').toArray()) {
        $('#course-list').append(`<li class="list-element group"><div>` + $(group).siblings('.category_label').text() + `</div><svg class="arrow" viewBox="0 0 6 5" xmlns="http://www.w3.org/2000/svg"><path d="M3 5l3-5H0z" fill="#FFF" fill-rule="evenodd"></path></svg></li>`);
        for ([key, value] of Object.entries(parseCourses(group))) {
            let labURL = value.lab ? '<a href="https://culearn.carleton.ca/moodle/course/view.php?id=' + value.lab + '"><div class="course_action">LAB</div></a>' : '';
            $('#course-list').append(`
                <li class="list-element course"><a href="https://culearn.carleton.ca/moodle/course/view.php?id=` + value.code + `"><div>` + key + ` ` + value.section + `</div></a><div class="course_actions">` + (labURL ? labURL : '') + `</div></li></a>
            `);
        }
    }
}
...
function parseCourses(group) {
    ...
    for (course of $(group).children().toArray()) {
        ...
        let name = TITLE.match(/[A-Z]{4}\d{4}/) ? TITLE.match(/[A-Z]{4}\d{4}/)[0] : TITLE.replace(/ *\([^)]*\)/g, "");
...
async function parseCalendar() {
    let calendarDat = await retrieveRaw('https://culearn.carleton.ca/moodle/calendar/view.php?view=upcoming');
...
<main>
<section class="lft">
    <div id='courses' class="section-wrapper courses-panel">
...
`);
    ...
    $('#page-load').hide();
    $('#custom-bg').fadeIn(500, function() {
        $("#custom-page").fadeTo("fast", 1);
    });
    ...
    $(document).on('keyup', '.search-input', function (e) {
        const VALUE = $('.search-input').val().toUpperCase().replace(/\s/g, '');
        for (course of $('.course').toArray()) {
            if ($(course).text().toUpperCase().replace(/\s/g, '').indexOf(VALUE) > -1) $(course).show();
            else $(course).hide();
        }
    });
});