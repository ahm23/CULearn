...
function parseCourses(group) {
    ...
    for (course of $(group).children().toArray()) {
        ...
        let name = TITLE.match(/[A-Z]{4}\d{4}/) ? TITLE.match(/[A-Z]{4}\d{4}/)[0] : TITLE.match(/^\((.*?)\)/)[1];
        if (TITLE.indexOf('LAB') === -1) {
            let section = TITLE.indexOf('Crosslist') > -1 ? 'Crosslist' : TITLE.match(/(?<=^[A-Z]{4}\d{4})(.)/g);
            let lab = courses && courses[name] ? courses.name.code : undefined;
            courses[name] = {
                code,
            ...
            if (courses && courses[name]) courses[name].lab = code;
            else {
                courses[name] = {
                    code,
                    section: 'LAB'
...
`);
    ...
    $(document).on('keyup', '.search-input', function (e) {
        ...
        for (course of $('.course').toArray()) {
            if ($(course).text().indexOf(VALUE) > -1) $(course).show();
            else $(course).hide();
        }
    });
});